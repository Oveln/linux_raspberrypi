...
pub trait UartPortOps {
    ...
    fn set_termios(_port: &UartPort, _new: &mut Ktermios, _old: &Ktermios);

    /// * @set_ldisc:    set the line discipline
    fn set_ldisc(_port: &UartPort, _arg2: &mut Ktermios);
    ...
    fn verify_port(uart_port: &UartPort, ser: &mut SerialStruct) -> i32;
...
impl<T: UartPortOps> Adapter<T> {
    unsafe extern "C" fn tx_empty_callback(
    ...
    unsafe extern "C" fn set_mctrl_callback(
        uart_port: *mut bindings::uart_port,
        mctrl: core::ffi::c_uint,
    ) {
        // let port = ManuallyDrop::new(UartPort(uart_port));
        let port = unsafe { UartPort::from_raw(uart_port) };
    ...
    unsafe extern "C" fn get_mctrl_callback(
        uart_port: *mut bindings::uart_port,
    ) -> core::ffi::c_uint {
        // let port = ManuallyDrop::new(UartPort(uart_port));
        let port = unsafe { UartPort::from_raw(uart_port) };
    ...
    unsafe extern "C" fn send_xchar_callback(
        uart_port: *mut bindings::uart_port,
        ch: core::ffi::c_char,
    ) {
        // let port = ManuallyDrop::new(UartPort(uart_port));
    ...
    unsafe extern "C" fn break_ctl_callback(
        uart_port: *mut bindings::uart_port,
        ctl: core::ffi::c_int,
    ) {
        // let port = ManuallyDrop::new(UartPort(uart_port));
    ...
    unsafe extern "C" fn set_termios_callback(
        ...
        // let port = ManuallyDrop::new(UartPort(uart_port));
        let port = unsafe { UartPort::from_raw(uart_port) };
        T::set_termios(port, Ktermios::from_raw(new), Ktermios::from_raw_const(old))
    }

    unsafe extern "C" fn set_ldisc_callback(
        uart_port: *mut bindings::uart_port,
        arg2: *mut bindings::ktermios,
    ) {
        // let port = ManuallyDrop::new(UartPort(uart_port));
        let port = unsafe { UartPort::from_raw(uart_port) };
        T::set_ldisc(port, Ktermios::from_raw(arg2))
    }
    ...
    unsafe extern "C" fn port_type_callback(
        uart_port: *mut bindings::uart_port,
    ...
    unsafe extern "C" fn request_port_callback(
        uart_port: *mut bindings::uart_port,
    ) -> core::ffi::c_int {
        // let port = ManuallyDrop::new(UartPort(uart_port));
        let port = unsafe { UartPort::from_raw(uart_port) };
    ...
    unsafe extern "C" fn config_port_callback(
        uart_port: *mut bindings::uart_port,
        arg2: core::ffi::c_int,
    ) {
        // let port = ManuallyDrop::new(UartPort(uart_port));
    ...
    unsafe extern "C" fn verify_port_callback(
        ...
        // let port = ManuallyDrop::new(UartPort(uart_port));
        let port = unsafe { UartPort::from_raw(uart_port) };
        T::verify_port(port, SerialStruct::from_raw(ser))
    }
    ...
    unsafe extern "C" fn poll_init_callback(
        uart_port: *mut bindings::uart_port,
    ) -> core::ffi::c_int {
        // let port = ManuallyDrop::new(UartPort(uart_port));
        let port = unsafe { UartPort::from_raw(uart_port) };
    ...
    unsafe extern "C" fn poll_put_char_callback(
        uart_port: *mut bindings::uart_port,
        ch: core::ffi::c_uchar,
    ) {
        // let port = ManuallyDrop::new(UartPort(uart_port));
    ...
    unsafe extern "C" fn poll_get_char_callback(
        uart_port: *mut bindings::uart_port,
    ) -> core::ffi::c_int {
        // let port = ManuallyDrop::new(UartPort(uart_port));
        let port = unsafe { UartPort::from_raw(uart_port) };
